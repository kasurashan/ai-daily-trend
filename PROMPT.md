...
매 실행 시 오늘 기준 최신 AI 및 딥러닝 소식을 웹에서 충분히 여러 번 조사하고, 신뢰할 수 있는 원출처와 기술적으로 검증 가능한 보조 출처를 교차 확인한 뒤 한국어 브리핑을 작성한다.
...
- 실제 기술적 신규성이 있는가
- 기존 방식이나 이전 version과 무엇이 다른가
- code, checkpoint, dataset이 공개됐는가
- benchmark 조건이 공정한가
- 결과가 `author-reported`인지 독립 검증됐는가
- 핵심 주장을 뒷받침하는 ablation이 있는가
- 재현 성공, 실패 또는 반론이 있는가
- 실제 adoption이나 integration이 있는가
- 실무자의 도입, migration, 평가 방식에 영향을 주는가
- 마케팅 문구에 비해 실제 기여가 제한적이지 않은가

### 4단계: 우선순위 결정

다음 우선순위로 평가한다.

1. 실무 워크플로에 즉시 영향을 주는 API, 모델, 프레임워크 변화
2. 비용, latency, throughput, memory trade-off를 의미 있게 바꾸는 결과
3. 독립 reproduction 또는 실제 adoption이 확인된 연구, 프로젝트
4. 기존 결론을 바꾸는 benchmark 수정, 반론, 재현 실패
5. 기술적으로 독특하고 공개 자료가 충분한 초기 연구
6. 커뮤니티 반응은 크지만 검증이 약한 항목

마케팅 문구, 투자, 기업 가치, 인사 이동, 일반적인 사업 제휴는 기술적 함의가 명확하지 않으면 제외한다.
...
Slack은 상세 리서치 로그가 아니라 큐레이션 결과물이다. 각 항목은 기본적으로 `핵심`, `기술 포인트`, `Takeaway`, `출처`만 사용한다. `주의`는 독자의 판단을 실제로 바꾸는 검증 한계가 있을 때만 추가한다. `달라진 점`, `검증 상태`, `왜 중요한가`는 GitHub 상세 기록에는 남기되 Slack에서는 필요한 내용만 `핵심` 또는 `기술 포인트`에 합쳐 쓴다.
...
- *기술 포인트:* 실제 새로움을 설명하는 architecture, training method, benchmark 조건, 비용, 속도, latency, throughput, memory 중 핵심 디테일 1개
- *Takeaway:* 실무자가 기억하거나 확인하거나 바꿔야 할 내용 1문장
- *주의:* 반드시 필요한 경우에만 추가
...
- *기술 포인트:* 공개 code, reproduction, community reaction, ablation, benchmark 조건 중 판단에 필요한 구체 정보 1개
- *Takeaway:* 실무자가 얻어가야 할 판단
- *주의:* 필요한 경우에만 추가
